...
const Portfolio = () => {
  ...
  return (
    <section id="portfolio" className="py-16 md:py-24 bg-sthavia-charcoal overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div ref={headerAnimation.ref} className={`text-center mb-16 ${headerAnimation.className}`}>
          ...
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-light text-sthavia-soft-white mb-6 leading-tight">
            Featured
            <span className="block font-medium text-sthavia-gold">Projects</span>
          </h2>
        ...
        <div ref={containerRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {projects.map((project, index) => (
            <Card
              key={project.id}
              className={`group overflow-hidden hover:shadow-xl transition-all duration-500 hover:transform hover:-translate-y-1 border-0 shadow-md bg-sthavia-primary-green/50 backdrop-blur-sm hover:bg-sthavia-primary-green/70 ${getItemClassName(index)}`}
            >
              <div className="relative overflow-hidden">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-sthavia-deep-forest/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="text-sthavia-soft-white font-medium px-6 py-3 border-2 border-sthavia-gold rounded-full hover:bg-sthavia-gold hover:text-sthavia-deep-forest transition-all duration-300 cursor-pointer transform translate-y-4 group-hover:translate-y-0">
                      View Project
                    </span>
                  </div>
                </div>
                <div className="absolute top-4 left-4 transform -translate-x-8 group-hover:translate-x-0 transition-transform duration-500">
                  <span className="bg-sthavia-gold text-sthavia-deep-forest px-3 py-1 rounded-full text-sm font-medium">
                    {project.category}
                  </span>
                </div>
              </div>
              <CardContent className="p-6">
                <h3 className="text-xl font-semibold text-sthavia-soft-white mb-2 group-hover:text-sthavia-gold transition-colors duration-300">
                  {project.title}
                </h3>
                <p className="text-sthavia-soft-white/70 group-hover:text-sthavia-soft-white/90 transition-colors duration-300">
                  {project.description}
                </p>
              </CardContent>
            </Card>
        ...
        <div ref={buttonAnimation.ref} className={`text-center ${buttonAnimation.className}`}>
          <Button
            size="lg"
            className="bg-sthavia-gold hover:bg-sthavia-gold/90 text-sthavia-deep-forest px-8 py-4 text-lg font-semibold transition-all duration-300 transform hover:scale-105"
          >
            View All Projects
          </Button>
        </div>
